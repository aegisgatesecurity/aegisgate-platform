...
func DefaultPatterns() []*Pattern {
	return []*Pattern{
		...
		{Name: "secret_uuid_token", Regex: regexp.MustCompile(`(?i)(?:consul|vault|nomad)\s+(?:acl\s+)?token\s*(?:is\s*[:=]|[:=])?\s*['"]?\b([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\b['"]?`), Severity: High, Category: CategoryCredential, Description: "HashiCorp Consul/Vault/Nomad UUID token"},
