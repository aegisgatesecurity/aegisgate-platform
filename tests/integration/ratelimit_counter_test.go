...
func TestRateLimitCounter_TierDifferentiation(t *testing.T) {
	// Tier RPMs: Community=-1 (unlimited, soft-throttle), Developer=500,
	// Professional=5000, Enterprise=-1 (unlimited). See tier.RateLimitMCP().
	tests := []struct {
		name        string
		t           tier.Tier
		rpm         int
...
func TestRateLimitCounter_WindowReset(t *testing.T) {
	// Community is unlimited (-1); use Developer (500 RPM) for rate-limit testing.
	cfg := mcpserver.DefaultGuardrailConfig(tier.TierDeveloper) // 500 RPM
	g := mcpserver.NewGuardrailMiddleware(cfg, "test-server")
